...
@Component({
  ...
  template: `
    <div class="container schedule-page"><div class="page-header"><h1>Exam Schedule</h1><p>Create an exam timetable and assign eligible students.</p></div>
      <form class="glass-panel schedule-form" (ngSubmit)="save()">
        ...
        <section class="students"><h3>Assign students</h3><label class="check" *ngFor="let student of students"><input type="checkbox" [checked]="selected.has(student.id)" (change)="toggle(student.id)">{{student.fullName}} <small>{{student.enrollmentNumber}}</small></label></section>
  ...
  styles: [`.schedule-page{padding-bottom:48px}.schedule-form{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px;padding:28px}.schedule-form label{display:flex;flex-direction:column;gap:7px;color:var(--uamp-text-muted);font-size:.85rem}.check{flex-direction:row!important;align-items:center}.students{grid-column:1/-1;display:grid;gap:10px}.students h3{color:var(--uamp-text-primary)}.students small{color:var(--uamp-text-muted)}.success{color:var(--uamp-accent-success)}@media(max-width:700px){.schedule-form{grid-template-columns:1fr}}`]
})
